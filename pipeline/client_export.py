...
from mta_delay_insights.sources.gtfs_static import NY_TZ, StaticGTFS, rt_trip_suffix


def export_client_schedule(static: StaticGTFS, resolved: list[dict], journeys: list, out_data: Path, now: datetime,
                           feeds: list[str] | None = None, feed_urls: dict[str, str] | None = None, demo_now: float | None = None) -> dict:
    ...
    feed_keys = sorted(feeds or {config.feed_for_route(r) for r in routes_needed if r in {x for v in config.SUBWAY_FEED_ROUTES.values() for x in v}})
    out = {"generated_at": now.isoformat(), "service_date": sd.isoformat(), "targets": targets, "lines": lines,
           "feeds": {k: (feed_urls or {}).get(k) or config.rt_feed_url(k) for k in feed_keys}, "alerts_url": config.rt_feed_url("subway_alerts_json"),
           "demo_now": demo_now,
