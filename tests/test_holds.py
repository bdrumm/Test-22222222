...
def test_hold_summary_and_alert_latency(static, tmp_path):
    ...
    seq = static.canonical_stop_sequence("6", "N"); origin, last = seq[0], seq[-1]
    terminal_wait = pd.DataFrame([{"trip_key": "t|0", "route_id": "6", "direction": "N", "stop_id": origin, "stopped_from_ts": T0 + 100, "stopped_to_ts": T0 + 1000, "dwell_sec": 900, "polls": 30},
                                  {"trip_key": "t|1", "route_id": "6", "direction": "N", "stop_id": last, "stopped_from_ts": T0 + 100, "stopped_to_ts": T0 + 700, "dwell_sec": 600, "polls": 20}])
    s = hold_summary(pd.concat([holds, terminal_wait], ignore_index=True), alerts, static)
    assert s["n_terminal"] == 2 and all(x["stop_id"] not in (origin, last) for x in s["by_stop"]), "waiting at either terminal is not a hold"
